...
def render_market_page(*, title: str, section: dict, checklist_title: str, hub_title: str, master_graph: dict | None = None, market_key: str | None = None) -> None:
    ...
    with live_col:
        render_opportunity_table(top_live[:6], 'Top opportunities now', cols=['ticker','bias','horizon','countdown_days_left','review_state','next_action','route_source_label','ev_score'], empty_msg='Belum ada top opportunities live untuk market ini.')
    with next_col:
        render_opportunity_table(top_next[:6], 'Next opportunities', cols=['ticker','bias','horizon','countdown_days_left','review_state','next_action','route_source_label','ev_score'], empty_msg='Belum ada next opportunities untuk market ini.')
