...
def run_ihsg_native_engine(raw: dict, shared_core: dict, features: dict, macro_board: dict, transmission: dict) -> dict:
    ...
    for bucket, syms in IHSG_BUCKETS.items():
        ranked_strong, _ = rank_symbols(prices, syms, top_n=1, context=ranking_ctx)
...
def _setup_row(r, side):
    leaders = {'BBCA', 'BBRI', 'BMRI', 'ADRO', 'PTBA', 'ITMG', 'AADI', 'ANTM', 'MDKA'}
    return {
        'name': r['name'],
        'bucket': 'IHSG',
        'side': side,
        'score': round(r['score'], 3),
        'why_now': f"r21 {r['r21']:.1%} · r63 {r['r63']:.1%} · eff {r['efficiency']:.2f} · ctx {r['context_adj']:.2f}",
        'action': classify_action(r['score'], side=side),
        'invalidator': 'USD/IDR naik lagi / breadth rusak' if side == 'long' else 'breadth recover / foreign sell reda',
        'risk': 'High' if r['vol21'] > 0.05 or r['exhaustion'] > 0.55 else 'Medium',
        'setup_type': (
            'Banks/Resources Leader' if side == 'long' and r['name'] in leaders
            else ('Vulnerable Laggard' if side == 'short' else 'Selective Leader')
        ),
    }
...
def _radar_row(r, side):
    return {
        ...
        'not_ready': 'tunggu breadth/flow confirm' if side == 'long' else 'tunggu failed bounce / breadth tetap jelek',
        'trigger': 'foreign flow stabil + breadth membaik' if side == 'long' else 'foreign flow tetap jelek + price gagal rebound',
