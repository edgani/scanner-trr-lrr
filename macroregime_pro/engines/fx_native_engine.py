...
def run_fx_native_engine(raw: dict, shared_core: dict, features: dict, macro_board: dict, transmission: dict) -> dict:
    ...
    strong, weak = rank_symbols(prices, all_symbols, top_n=14, context=ranking_ctx)
...
def _setup_row(r, side: str):
    return {'name':r['name'],'bucket':'FX','side':side,'score':round(r['score'],3),'why_now':f"r21 {r['r21']:.1%} · r63 {r['r63']:.1%} · eff {r['efficiency']:.2f} · ctx {r['context_adj']:.2f}",'action':classify_action(r['score'], side),'invalidator':'macro surprise berbalik / intervention','risk':'High' if r['vol21'] > 0.05 else 'Medium','setup_type':'Clean Divergence' if side=='long' else 'Weak / Fragile Pair'}


def _radar_row(r, side: str):
    return {'name':r['name'],'bucket':'FX','side':side,'score':round(r['score'],3),'why_radar':f"trend {r['trend']:.2f} · eff {r['efficiency']:.2f} · base {r['base_score']:.2f}",'not_ready':'butuh macro surprise confirm','trigger':'data makro / rate repricing','risk':'Whipsaw','radar_type':'Almost Ready' if side=='long' else 'Short Radar'}
