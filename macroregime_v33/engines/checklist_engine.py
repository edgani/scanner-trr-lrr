...
def build_global_checklist(shared_core: dict, features: dict, news_state: dict, em_rotation: dict) -> list[dict]:
    ...
    benign_news_states = {"quiet", "relief", "deescalation_watch", "deescalation_confirmed", "oil_shock_fading"}
    news_is_benign = str(news_state.get("state","quiet")) in benign_news_states
    items=[("Growth",0.5+0.5*float(macro.get("growth_momentum",0.0))),("Inflasi",0.5-0.35*float(macro.get("inflation_shock",0.0))),("DXY",0.5-0.5*max(0.0,float(market.get("dxy_1m",0.0)))),("Yields",0.5+0.5*float(market.get("tlt_1m",0.0))),("Breadth",float(market.get("breadth_health",0.5))),("Vol/Credit",0.6 if shared_core.get("vix_bucket",{}).get("bucket") in {"calm","normal"} else 0.3),("Liquidity",0.55 if shared_core.get("execution_mode",{}).get("mode") != "defensive" else 0.35),("Geopolitics",0.65 if news_is_benign else 0.35),("EM Rotation",float(em_rotation.get("score",0.4))),("Event/News",0.65 if news_is_benign else 0.4)]
    out=[]
