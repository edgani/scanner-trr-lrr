...
def render_execution_map_panel(section: dict) -> None:
    ...
    pills = [(str(bias), 'blue'), (str(mode), tone), (f"Score {score:.2f}", 'blue')]
    flags = section.get('flags', {}) or {}
    if flags.get('no_trade'):
        pills.append(('No Trade', 'bad'))
    elif flags.get('can_chase'):
        pills.append(('Can Chase', 'good'))
    elif flags.get('buy_dip_only'):
        pills.append(('Buy Dip Only', 'warn'))
    if flags.get('reduce_on_strength'):
        pills.append(('Reduce Strength', 'warn'))
    if flags.get('short_bounces_only'):
        pills.append(('Short Bounces', 'bad'))
    render_pills(pills)
