...
from features.ihsg_structural_features import build_ihsg_structural_state
...
def build_ihsg_native_features(raw: dict, shared_core: dict) -> dict:
    ...
    structural_state = build_ihsg_structural_state(shared_core, {
    ...
    clean_float_rotation = clamp01(structural_state.get('clean_float_rotation_score', 0.0))

    execution_score = clamp01(
        0.23 * global_risk
        + 0.19 * foreign_flow
        + 0.14 * breadth_liquidity
        + 0.14 * bank_health
        + 0.12 * commodity_spillover
        + 0.08 * (1.0 - usd_idr_pressure)
        + 0.10 * clean_float_rotation
    )
    execution_state = {
    ...
    return {
        ...
        'clean_float_rotation_score': clean_float_rotation,
